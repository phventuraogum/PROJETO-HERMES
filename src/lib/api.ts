...
async function hermesFetch<T>(path: string, opts: (HermesFetchOptions & { skipOrgHeader?: boolean }) = {}): Promise<T> {
  const token = await getAuthToken();

  const headers = new Headers(opts.headers || {});
  if (token) headers.set("Authorization", `Bearer ${token}`);
  if (!opts.skipOrgHeader && typeof window !== "undefined" && !headers.has("X-Org-Id")) {
...
export async function apiFetch<T>(path: string, opts: ApiFetchOptions = {}): Promise<T> {
  ...
  return hermesFetch<T>(path, { ...rest, headers, skipOrgHeader });
}
...
export async function getKommoIntegration(orgId: string): Promise<KommoIntegration> {
  return apiFetch<KommoIntegration>(`/orgs/${encodeURIComponent(orgId)}/integrations/kommo`, { skipOrgHeader: true });
}
